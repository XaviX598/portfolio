...
export default function Contact({ lang }: ContactProps) {
  ...
  return (
    <section id="contact" className="w-full h-full min-h-screen relative flex items-center justify-center">
      <div className="absolute inset-0 pointer-events-none bg-gradient-to-t from-zinc-950 via-zinc-950/50 to-transparent" />

      <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 py-16 relative z-10">
        <FadeIn delay={0.1}>
          <div
            data-inview
            className="mt-8 rounded-2xl border border-white/10 bg-zinc-900/70 backdrop-blur p-6 sm:p-8 md:p-10"
          >
            ...
            {isSuccess ? (
              <div className="p-4 rounded-xl bg-teal-500/20 border border-teal-500/30 text-teal-300 text-center">
                {t.success}
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    {contactMethod === "whatsapp" ? (
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-xl bg-zinc-800/50 border border-white/10 text-white placeholder-zinc-500 focus:border-teal-500 focus:outline-none transition-colors"
                      />
                    ) : (
                      <input
                        type="email"
                ...
                <button
                  ...
                  className="w-full py-4 rounded-xl bg-teal-500 hover:bg-teal-400 text-white font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
